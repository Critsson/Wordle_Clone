...
function checker() {
    ...
    if(!dictionary.includes(input.join("").toLowerCase())) {
        createAlert("Not a word");
    ...
    for (i = 0; i < 5; i++) {
        if (input[i] === answer.charAt(i)) {
            ...
            document.querySelector(`.${input[i].toLowerCase()}`).classList.add("greenFast");
            input[i] = "";
            greenCounter++;
        }
    }
    ...
    for (i = 0; i < 5; i++) {
        if (answer.includes(input[i]) && answerArr.includes(input[i])) {
            ...
            document.querySelector(`.${input[i].toLowerCase()}`).classList.add("yellowFast");
            answerArr.splice(answerArr.indexOf(`${input[i]}`), 1,);
        }
        else if (!answerArr.includes(input[i])) {
            document.getElementById(`box${verticalPointer}-${i}`).classList.add("darkGrey");
            if (input[i] != "") {
                document.querySelector(`.${input[i].toLowerCase()}`).classList.add("darkGreyFast");
            }
        }
    }
